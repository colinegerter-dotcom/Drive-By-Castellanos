    ...
    for i, g in enumerate(game_rows, 1):
        ...
        for pitcher_id in (g.get("home_starter_id"), g.get("away_starter_id")):
            ...
            pitcher_form_rows.append(
                build_starting_pitcher_form_row(conn, pitcher_id, game_id, game_date, season, debut_by_player.get(pitcher_id))
            )
        ...
        for batter_id in batter_ids:
            batter_form_rows.append(
                build_starting_batter_form_row(conn, batter_id, game_id, game_date, season, debut_by_player.get(batter_id))
            )
